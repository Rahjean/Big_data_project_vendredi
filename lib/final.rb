...
def gazety(x)
	puts "il y a #{x.length} journalistes"
end
...
def numero(x)
	journaliste = x.select {|chifre| chifre =~ /[1234567890]/ }
	puts "Il y a #{journaliste.length} handle qui contient un numero"
end


def aude(x)
	puts "Il y a #{x.grep(/[Aa]ude/).length} qui contient le Prenom Aude." 
end


def majuscule(x)
  maj = 0 #ilay isan'ilay @ aminilay @jcunniet

   for mot in x

	   	if mot.scan(/[A-Z]/).length == 1 #mot :  maka anlay valeur an tableau ,  .scan oe misy lettre majuscule de A a Z v ao anatilay mot ka raha misy dia executena ilay maj = maj + 1 
	   		maj = maj + 1 
	   	end

   end

   	puts "Il y a #{maj} handles qui commence par une majuscule"
...
def contient_maj(x)
	journaliste = x.each {|soratra| soratra.downcase!} # le ! indique le permanent
...
def under_score(x)
	journaliste = x.select {|under_score| under_score =~ /[_]/ }
	puts "Il y a #{journaliste.length} handle qui contient un under_score"
end


def alphabetique(x)
	ito = x.sort_by {|a| a.downcase}
	puts ito
end

def position(x)
	puts "La position dans l'array de la personne @epenser est au #{x.index("@epenser")}"
end
...
puts "veuillez choisir parmis ce choix svp :D"
...
if choix == 1	
    ...
    contient_maj(journaliste)

    elsif choix == 6
    under_score(journaliste)

    elsif choix == 7
	alphabetique(journaliste)

    elsif choix == 8
	position(journaliste)

    else puts "veuillez choisir entre ce choix svp"  	
